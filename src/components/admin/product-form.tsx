    ...
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select
                ...
                className="flex-1 px-4 py-3 bg-tecno-bg/60 border border-tecno-cyan/30 rounded-lg outline-none focus:border-tecno-cyan focus:ring-2 focus:ring-tecno-cyan/20 transition-all"
              >
                {categories.map((cat) => (
              ...
              <button
                ...
                className="px-4 py-3 bg-tecno-cyan/10 text-tecno-cyan rounded-lg hover:bg-tecno-cyan/20 transition-colors"
            ...
            {showAddCategory && (
              <div className="flex gap-2 mt-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-tecno-bg/60 border border-tecno-cyan/30 rounded-lg outline-none focus:border-tecno-cyan transition-all"
                />
                <button
                  type="button"
                  onClick={handleAddCategory}
                  className="px-4 py-2 bg-tecno-cyan text-white rounded-lg hover:opacity-90 transition-opacity"
                >
                  Agregar
                </button>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-5 h-5 rounded border-tecno-cyan/30 text-tecno-cyan focus:ring-tecno-cyan/20"
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-5 h-5 rounded border-tecno-cyan/30 text-tecno-cyan focus:ring-tecno-cyan/20"
              />
              <span className="text-sm">🎠 En Carrusel</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
        ...
        <div className="space-y-4">
          {formData.images.map((img, index) => (
            <div key={index} className="flex gap-3 items-start">
              ...
              {img && (
                <div className="w-20 h-20 rounded-lg overflow-hidden border border-tecno-cyan/30 flex-shrink-0">
                  <Image
                    src={img}
                    alt={`Preview ${index + 1}`}
                    width={80}
              ...
              {formData.images.length > 1 && (
                <button
                  ...
                  className="px-3 py-3 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition-colors"
                  title="Eliminar"
                >
                  🗑️
                </button>
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="flex gap-3 mb-4">
          <input
            ...
            className="flex-1 px-4 py-3 bg-tecno-bg/60 border border-tecno-cyan/30 rounded-lg outline-none focus:border-tecno-cyan transition-all"
          />
          <input
            type="text"
            value={specValue}
          ...
          <button
            type="button"
            onClick={addSpec}
            className="px-6 py-3 bg-tecno-cyan text-white rounded-lg hover:opacity-90 transition-opacity"
          >
            ➕ Agregar
          </button>
        </div>
        ...
        <div className="space-y-2">
          {Object.entries(formData.specs).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between bg-tecno-bg/30 rounded-lg p-3">
      ...
      <div className="flex gap-4">
        <button
          type="button"
          onClick={() => router.back()}
          className="px-6 py-3 bg-tecno-bg/60 border border-tecno-cyan/30 rounded-lg hover:bg-tecno-bg/80 transition-colors"
        >
          Cancelar
        </button>
        <button
